...
def get_brats2021_train_transform_abnormalty(image_size):
    base_transform = [
        transforms.EnsureChannelFirstd(
            keys=['input', 'brainmask', 'gauss_mask'], channel_dim='no_channel'),
        transforms.Resized(
            keys=['input', 'brainmask', 'gauss_mask'],
...
class BraTS2021Dataset(Dataset):
    ...
    def __getitem__(self, index: int) -> tuple:
        ...
        rv = multivariate_normal([mean1, mean2], [[covar11, covar12], [covar12, covar22]], allow_singular=True)
        gau_pdf = rv.pdf(pos)
        ...
        item = self.transforms(
            {'input': input, 'brainmask': brain_mask,  'gauss_mask': gauss_mask})
