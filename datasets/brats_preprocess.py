...
def nii2np_test(img_root, img_name, upper_per=0.9, lower_per=0.02, output_root_test='/mount/src/dashboard_medical/datasets/data/test', modality=['t1']):
    ...
    for mod_num in range(len(modality)):
        ...
        img = (img.get_fdata())
        img_original = img
        for slice_id in range(img.shape[2]):
            ...
            dirs_mod = os.path.join(output_root_test, patient_id, str(slice_id), modality[mod_num])
            ...
            dirs_brainmask = os.path.join(output_root_test, patient_id, str(slice_id), 'brainmask')
